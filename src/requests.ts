...
export function createUpdateRequest<T extends Item, K extends KeyAttributes<T>, B extends Item, T2 extends T = T, R extends ReturnValue = 'new'>(
  ...
  const attr = {};
  const {key, attributes, conditions, returnValues = 'new'} = params;
  Object.assign(attributes, ...model.params.updaters.map(updater => updater(attributes)));

  return {
    TableName: model.tableName,
    Key: key,
    ReturnValues: returnValues === 'old' ? 'ALL_OLD' : 'ALL_NEW',
